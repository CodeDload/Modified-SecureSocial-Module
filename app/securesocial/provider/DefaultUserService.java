...
public class DefaultUserService implements UserService.Service {
    ...
    private Map<String, SocialUser> pwResets = Collections.synchronizedMap(new HashMap<String, SocialUser>());
    ...
    public String resetPasswordRequest(SocialUser user) {
        final String uuid = Codec.UUID();
        user.isPasswordBeingReset = true;
        user.password="INVALID_PASSWORD";        
        pwResets.put(uuid, user);
        return uuid;
    }

    public String resetPasswordVerify(String uuid) {
        SocialUser user = pwResets.get(uuid);
        String userName = null;

        if( user != null ) {
            //  password reset link should become inactive after this
            pwResets.remove(uuid);            
            userName = user.id.id;
        }
        return userName;
    }

    public void deletePendingPasswordResets() {
        pwResets.clear();
    }
